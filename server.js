...
function buildResponsesPreviewRows(schema, rows) {
  ...
  const headers = ["updatedAt", "name", "group", "perPerson"].concat(dynamicColumns.map(column => column.label));
...
app.get("/api/responses", async (req, res) => {
  try {
    ...
    const schema = await getSetting("formSchema", []);
    const result = await query(
      `SELECT updated_at, name, group_name, per_person, answers FROM ${TABLES.submissions} ORDER BY updated_at DESC LIMIT 100`
    );
...
app.post("/api/submissions/upsert", async (req, res) => {
  try {
    ...
    `, [
      ...
      JSON.stringify(deepParse(payload.answers, {})),
